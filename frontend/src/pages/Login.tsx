...
const Login = () => {
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setLoginInfoData(id as keyof LoginInfoI, value);
  };
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.status === "success") {
        ...
        document.cookie = `loginToken=${token};expires=${expirationTime.toISOString()};path=/*`;
        navigate("/home");
      }
    } catch (error: unknown) {
  ...
  const { email, password } = loginInfoStore((state) => state);

  const setLoginInfoData = loginInfoStore((state) => state.setLoginInfo);
  return (
    <div className="w-screen h-screen  ">
      <div className="min-w-[500px] w-screen h-[720px] bg-[#f2f4f7] flex  justify-center pt-[72px]  pb-[112px] ">
        <div className=" flex w-[980px] ">
          <div className=" flex  flex-col pb-36 py-28">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="300"
              height="98"
              fill="none"
              viewBox="100 0 1050 300"
            >
              <path
                fill="#0866FF"
            ...
            <h2 className="text-[28px] leading-[28px] text-[#1c1e21] w-5/6 px">
              Connect with friends and the world around you on Facebook.
            </h2>
          </div>
          <div className="flex flex-col items-center pt-16  radius">
            <div className="w-[396px]  p-[16px] h-fit  bg-white rounded-[5px] shadow-md">
              <form
              ...
              >
                <div className=" flex flex-col  gap-3">
                  <input
                    className={`border border-[#ccd0d5]  rounded-[5px] h-[52px] text-[16px] px-[18px]  w-full`}
                ...
                <button className="w-full bg-[#0866ff] mt-2 text-white text-2xl py-2 rounded-[5px] font-semibold">
                  Log In
                </button>

                <Link to="/reset-password" className="w-full mb-2">
                  <h4 className=" text-center text-blue-600  font-semibold">
                    Forgot password?
                  </h4>
                </Link>

                <div className="w-full flex justify-center  border-t border-t-[#dadde1] pt-6 pb-2">
                  <div className="w-[195px] bg-[#42b72a] text-white text-center leading-[48px] rounded-[5px]">
